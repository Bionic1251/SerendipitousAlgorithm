...
public class SerFunkSVDItemScorer extends AbstractItemScorer {
	protected final SerFunkSVDModel model;
	 ...
	 * @param baseline The genrePredictor scorer.  Be very careful when configuring a different genrePredictor
	 ...
	 */
	@Inject
	public SerFunkSVDItemScorer(UserEventDAO dao, SerFunkSVDModel model,
	 ...
	 *               be initialized to the user's genrePredictor predictions.
	 */
	private void computeScores(long user, AVector uprefs, MutableSparseVector output) {
		for (VectorEntry e : output) {
			...
			if (ivec == null) {
				...
				// unset the genrePredictor to note that we are not predicting for this item.
				output.unset(e);
			} else {
