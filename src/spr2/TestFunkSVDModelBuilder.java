 ...
 */
public class TestFunkSVDModelBuilder implements Provider<TestFunkSVDModel> {
	private static Logger logger = LoggerFactory.getLogger(TestFunkSVDModelBuilder.class);
	...
	protected final TestFunkSVDUpdateRule rule;

	@Inject
	public TestFunkSVDModelBuilder(@Transient @Nonnull PreferenceSnapshot snapshot,
								   @Transient @Nonnull TestFunkSVDUpdateRule rule,
	...
	public TestFunkSVDModel get() {
		...
		TestTrainingEstimator estimates = rule.makeEstimator(snapshot);
		...
		double betta = rule.getTrainingRegularization();
		double sum;
		for (int j = 0; j < 300; j++) {
			sum = 0;
			for (IndexedPreference rating : snapshot.getRatings()) {
				...
				for (int i = 0; i < featureCount; i++) {
					double val = item.get(i) + alpha * (2 * error * user.get(i) - betta * item.get(i));
					...
					val = user.get(i) + alpha * (2 * error * item.get(i) - betta * user.get(i));
					if (Double.isNaN(val) || Double.isInfinite(val)) {
						System.out.println("NaN");
					}
					user.set(i, val);
		...
		return new TestFunkSVDModel(ImmutableMatrix.wrap(userFeatures),
